...
import { signup, setToken } from "../services/api.js";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f0f10", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20 }}>
      ...
      <div style={{ background: "#161618", border: "1px solid #2a2a2e", borderRadius: 18, padding: "36px 40px", width: "100%", maxWidth: 420 }}>
        <h1 style={{ color: "#fff", margin: "0 0 6px", fontSize: 24, fontWeight: 800 }}>Create your account</h1>
        <p style={{ color: "#666", margin: "0 0 28px", fontSize: 14 }}>Free forever — no credit card needed</p>

        <form onSubmit={onSubmit} style={{ display: "grid", gap: 16 }}>
          <div>
            <label style={{ color: "#888", fontSize: 13, display: "block", marginBottom: 6 }}>Full name</label>
            <input value={name} onChange={e => setName(e.target.value)} required style={inputStyle} placeholder="Vansh Kumar" />
          </div>
          <div>
          ...
          <div>
            <label style={{ color: "#888", fontSize: 13, display: "block", marginBottom: 6 }}>Password <span style={{ color: "#555" }}>(min 6 chars)</span></label>
            <input value={password} onChange={e => setPassword(e.target.value)} type="password" required minLength={6} style={inputStyle} placeholder="••••••••" />
          </div>
        ...
        <p style={{ color: "#555", marginTop: 20, fontSize: 13, textAlign: "center" }}>
          ...
          <Link to="/login" style={{ color: "#81c784", fontWeight: 600 }}>Sign in</Link>
        </p>
      </div>
    </div>
  );
